<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <label for="name" class="block text-sm font-semibold text-gray-700">{{ __('app.auth.name') }}</label>
            <div class="mt-1 relative rounded-xl shadow-sm group">
                ...
                <input id="name" name="name" type="text" class="pl-10 block w-full rounded-xl border-gray-200 bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all duration-200 sm:text-sm py-2.5" value="{{ old('name', $user->name) }}" required autofocus autocomplete="name" />
            </div>
            <x-input-error class="mt-2" :messages="$errors->get('name')" />
        </div>

        <div>
            <label for="email" class="block text-sm font-semibold text-gray-700">{{ __('app.auth.email') }}</label>
            <div class="mt-1 relative rounded-xl shadow-sm group">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                <input id="email" name="email" type="email" class="pl-10 block w-full rounded-xl border-gray-200 bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all duration-200 sm:text-sm py-2.5" value="{{ old('email', $user->email) }}" required autocomplete="username" />
            </div>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div class="mt-3 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700 font-medium">
                                ...
                                <button form="send-verification" class="ml-1 underline text-yellow-800 hover:text-yellow-900 focus:outline-none">
                    ...
                    @if (session('status') === 'verification-link-sent')
                        <div class="mt-2 text-sm text-green-600 font-medium flex items-center">
